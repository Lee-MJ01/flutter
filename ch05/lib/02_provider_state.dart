...
void main() {
  runApp(
    // 외부 라이브러리 provider 의존성 설정 : pubspec.yaml > provider: ^6.1.5+1추가(provider는 pub.dev에서 검색)
...
class _ParentWidgetState extends State<ParentWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Text('Provider count : ${counterProvider._count}',
...
class ChildWidget1 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Text('Child1 Provider count : ${counterProvider._count}',
          style: Theme.of(context).textTheme.headlineMedium,
        ),
        const SizedBox(height: 10,),
        Row(
  ...
  Widget build(BuildContext context) {
    return Consumer<CounterProvider>(
      builder: (context, notifier, child) {
        return Column(
          children: [
            Text('Child2 Provider count : ${notifier._count}',
              style: Theme.of(context).textTheme.headlineMedium,
            ),
            const SizedBox(height: 10,),
            Row(
  ...
  Widget build(BuildContext context) {
    return Consumer<CounterProvider>(
        builder: (context, notifier, child) {
          return Column(
            children: [
              Text('Child3 Provider count : ${notifier._count}',
                style: Theme.of(context).textTheme.headlineMedium,
              ),
              const SizedBox(height: 10,),
              Row(
