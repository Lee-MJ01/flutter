...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('05.Dialog 위젯 실습'),
        ),
        body: DialogTest(),
      ),
    );
...
class DialogTest extends StatelessWidget {
  ...
  void _showAlertDialog(BuildContext context){
    showDialog(
      context: context,
      barrierDismissible: false, // 배경 영역을 터치했을 때 대화상자 닫기 불가(false), 가능(true, Default)
      builder: (BuildContext context){
        return AlertDialog(
          title: const Text('기본 대화 상자'),
  ...
  void _showDateDialog(BuildContext context) async {
    DateTime? selectedDate = await showDatePicker( // 날짜는 선택 안 할수도 있어서 Nullable
                              context: context,
