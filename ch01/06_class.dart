...
class User {
  ...
  late String _email; // late를 선언하지 않으면 기본 생성자에 추가시켜줘야함. 아니면 에러 발생

  // 기본 생성자
  User(this._name, this._birth);

  // 명명된 생성자(Named Constructor), 초기화 목록 생성자 -> (Class Name).(사용자 정의 생성자 이름) : 초기화 {}
  ...
  int? get age => _age; // nullable이기에 똑같이 ? 사용
...
void main() {
  ...
  print('나이 : ${user1.age}'); // _name 직접 접근 안됨, age Getter 호출

  // Setter 사용
  user1.age = 30;
  print(user1);

  var user2 = User.withAge('김유신', '1999-02-15', 26);
  user2.initEmail('kim@naver.com');
  user2.hello();

  var user3 = User.guest('김춘추');
  user3.initEmail('kimch@naver.com');
  user3.hello();

  var user4 = User.withAge('장보고', '1999-02-15', 30);
  user4.initEmail('kim@naver.com');
  user4.hello();

  // 클래스 속성
  print('WithAge 전체인원 : ${User.total}');
